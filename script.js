...
class Bird {
  constructor() {
    ...
    this.image.src = "src/bird.png"; // Установка пути к изображению птицы
...
function endGame() {
  endGameDialog.classList.add("end-game__open"); //включаем диалоговое окно с результатами и кнопкой перезапуска
  score.innerHTML = bird.point; //устанавливаем заработанное количество очков
}

btn_restart.addEventListener("click", function () {
  endGameDialog.classList.remove("end-game__open"); //удалили класс, чтобы спрятать окно
  bird.x = bird.start_x; //поставили птичку в точку старта
  bird.y = bird.start_y;
  bird.speed = 0; //сбросили скорость
  bird.point = 0; //сбросили очки

  pipe_one.top_pipe.x = first_x; //установили первую пару в точку старта
  pipe_one.bottom_pipe.x = first_x;

  pipe_two.top_pipe.x = first_x + (canvas.width + pipe_one.top_pipe.width) / 2;
  pipe_two.bottom_pipe.x =
    first_x + (canvas.width + pipe_one.top_pipe.width) / 2; //установили вторую пару в точку старта
...
loop(); //Запускаем игру, вызвав первый раз вручную loop
